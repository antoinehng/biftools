...
namespace BifCreator
{
    ...
    {
        static Mutex Mutex;
        public static bool LauchApplicationWithMutex(string nomapplication)
        {
            try
            {
                ProcessHelper.Mutex = Mutex.OpenExisting(nomapplication);
                return false;
            }
            catch
            {
                ProcessHelper.Mutex = new Mutex(true, nomapplication);
                return true;
            }
        }

        public static int ExecuteProcess(string fileName, string arguments, int timeout, out string standardOutput, out string standardError)
        {
            int exitCode;
